...
from ..repositories.product_repo import ProductRepository, ProductData # Import alias too
...
class ListingService:
    ...
    def list_item_on_marketplace(
        ...
        if not product_data:
             print(f"Service ERROR: Product/Group with identifier '{product_identifier}' not found.")
             return None
        ...
        try:
            print(f"Service: Submitting {product_data.__class__.__name__} '{product_identifier}' via {adapter.name} adapter...")
            ...
            try:
                ...
                print(f"Service CRITICAL ERROR: Failed to save listing record for {listing_id_on_marketplace} after successful submission! Error: {repo_err}")
            ...
            print(f"Service ERROR: Failed to submit listing for '{product_identifier}' on '{marketplace_name}'. Error: {e}")
            return None
...
if __name__ == '__main__':
    ...
    product_repo.add(InventoryProduct(_sku="TEST-SKU-01", _title="Test Widget", _price=10.0))
    product_repo.add(InventoryProduct(_sku="TEST-SKU-02", _title="Another Widget", _price=15.0))
    ...
    listing_service = ListingService(product_repo, marketplace_adapters)

    print("\n--- Running Service Logic ---")
    # List item TEST-SKU-01 on MockPlace
    list_id1 = listing_service.list_item_on_marketplace("TEST-SKU-01", mock_adapter.name)
    print(f"Resulting Listing ID 1: {list_id1}")

    # List item TEST-SKU-02 on MockPlace
    list_id2 = listing_service.list_item_on_marketplace("TEST-SKU-02", mock_adapter.name)
