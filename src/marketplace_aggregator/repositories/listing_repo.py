...
from advanced_alchemy.filters import FilterTypes # Useful for filtering/deleting
...
class ListingRepository(SQLAlchemyAsyncRepository[Listing], ListingRepositoryInterface):
    ...
    async def add(self, data: Listing) -> Listing:
    ...
    async def get_by_composite_id(self, marketplace_name: str, marketplace_listing_id: str) -> Optional[Listing]:
    ...
    async def delete_by_composite_id(self, marketplace_name: str, marketplace_listing_id: str) -> Optional[Listing]:
        # 1. Fetch the item by the composite key to get its primary key (id)
        item_to_delete = await self.get_by_composite_id(marketplace_name, marketplace_listing_id)
        if item_to_delete and item_to_delete.id is not None:
            # 2. Use the base repository's delete method with the primary key
            deleted_id = await super().delete(item_to_delete.id)
