...
class VariableProduct:
    ...
    def get_variants_by_attributes(self, attrs_to_match: VariantAttributes) -> list[Sellable]:
        ...
        for variant in self.variants.values():
            if hasattr(variant, 'attributes') and isinstance(variant.attributes, dict):
                ...
                if all(variant.attributes.get(k) == v for k, v in attrs_to_match.items()):
...
class VariableProductBuilder:
    def __init__(self, group_id: str, title: str):
        ...
        print(f"Builder initialized for VariableProduct Group {group_id} ('{title}')...")
    ...
    def build(self) -> VariableProduct:
        print(f"--- Building VariableProduct Group: {self._variable_product.group_id} ---")
...
if __name__ == "__main__":
    ...
    variable_product = builder.set_description("Soft and comfortable cotton.")\
        .add_shared_image("tshirt_model.jpg")\
        .add_inventory_variant( # Use convenience method
            sku="TS-CTN-RD-M", title="Cotton T-Shirt (Red, M)", price=19.99,
            attributes={"Color": "Red", "Size": "M"}, weight_kg=0.18
        )\
        .add_inventory_variant(
            sku="TS-CTN-RD-L", title="Cotton T-Shirt (Red, L)", price=19.99,
            attributes={"Color": "Red", "Size": "L"}, weight_kg=0.20
        )\
        .add_inventory_variant(
            sku="TS-CTN-BL-M", title="Cotton T-Shirt (Blue, M)", price=20.50,
            attributes={"Color": "Blue", "Size": "M"}, weight_kg=0.18
        )\
        .build()
