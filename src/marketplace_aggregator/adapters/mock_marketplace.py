...
class MockMarketplace(Marketplace):
    ...
    def submit_listing(self, item: Sellable | VariableProduct) -> str:
        ...
        print(f"{self.name} ({self._seller_id}): Received submit_listing for {identifier}")
        ...
        print(f"{self.name} ({self._seller_id}): Created listing {listing_id} for {identifier}")
        return listing_id
    ...
    def update_listing_price(self, listing_id: str, sku: str, new_price: float) -> None:
        ...
        print(f"{self.name} ({self._seller_id}): Simulating price update for listing '{listing_id}', SKU '{sku}' to {new_price:.2f}")
    ...
    def update_listing_stock(self, listing_id: str, sku_stock: Dict[str, int]) -> None:
        ...
        print(f"MockPlace ({self._seller_id}): Received update_listing_stock for {listing_id}")
        if listing_id not in self._listings:
        ...
        print(f"MockPlace ({self._seller_id}): Updating stock for listing '{listing_id}': {sku_stock}")
        for sku, stock in sku_stock.items():
            ...
            if not sku_exists:
                print(f"{self.name} WARNING: SKU {sku} not found in listing {listing_id} during stock update.")
    ...
    def get_orders(self, since: datetime) -> List[Dict[str, Any]]:
        ...
        print(f"MockPlace ({self._seller_id}): Received get_orders since {since.isoformat()}")
